  ...
  viewTrampoline:function(req, res){
	...
	console.log(docs);
		if(docs.length===0){
        		...
        		request.validdate = req.body.validdate;
       			request.description = req.body.description;
       			 ...
       			 request.save(function(err){
            			 if (err) {
						console.log(err);
				            x= "There is an already Existing Request with the same mail";
                            res.json(x);
                			//res.json({success:true,message: 'There is an already Existing Request with the same mail'});
                		}else{
                               x = "Your Business Request is Sent to the Admin";
                               res.json(x);
			...
			x = "There is an already Existing Service Provider with the same mailT";
            res.json(x);
			//res.json({success:true,message: 'There is an already Existing Service Provider with the same mail'});
			}
	});
