...
viewBooking:  function(req, res){
        bookingHistory.find({userEmail: 'reem@gmail.com' /*req.user.email*/}, function(err, docs){ //Write the condition you want to display the bookings table from which is email: "Webtoken email"
            if(err) res.json(err);
            else res.json(docs);
        });
